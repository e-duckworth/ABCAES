% this function appplies the whole encryption algorithm.  "stream"
% refers to the fact that the input does not have to be a single
% block.
function ciphertext = ABCstream(plaintext,key,verbose)
...
if debug,K0,K1,K2,disp('Next add K0'),end
...
ciphertext=SR(ciphertext);
   if debug
       ciphertext
       disp('Next apply MC')
       ciphertext=MC(ciphertext,'verbose');
   else
